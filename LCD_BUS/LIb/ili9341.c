...
void ILI9341_FillRectangle(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color) {
    // clipping
	/*
	//2 ham for long nhau khien no bi cham
	...
	*/
    ...
    static uint16_t lineBuf[ILI9341_WIDTH];
    ...
    for (uint16_t row = 0; row < h; row++) {
        HAL_SPI_Transmit(&ILI9341_SPI_PORT, (uint8_t*)lineBuf, w * 2, HAL_MAX_DELAY);
    }
...
void ILI9341_SetScrollArea(uint16_t top, uint16_t height, uint16_t bottom)
{
    ILI9341_Select();
    ILI9341_WriteCommand(0x33);  // Vertical Scrolling Definition

    uint8_t data[] = {
        (top >> 8) & 0xFF, top & 0xFF,
        (height >> 8) & 0xFF, height & 0xFF,
        (bottom >> 8) & 0xFF, bottom & 0xFF
    };
    ILI9341_WriteData(data, sizeof(data));
    ILI9341_Unselect();
}

void ILI9341_SetScrollStartAddr(uint16_t addr)
{
    ILI9341_Select();
    ILI9341_WriteCommand(0x37);               // Vertical Scrolling Start Address
    uint8_t data[] = { addr >> 8, addr & 0xFF };
    ILI9341_WriteData(data, sizeof(data));
    ILI9341_Unselect();
}


void ILI9341_DrawImageFromFlash(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint32_t startPage) {
    uint32_t total = w * h * 2;
    uint8_t buffer[512];
    uint32_t addr = 0;
    ...
    while (total > 0) {
        uint32_t chunk = (total > sizeof(buffer)) ? sizeof(buffer) : total;

        uint32_t page = startPage + (addr / 256);
        uint32_t offset = addr % 256;

        W25Q_Read(page, offset, chunk, buffer);
        HAL_SPI_Transmit(&ILI9341_SPI_PORT, buffer, chunk, HAL_MAX_DELAY);
